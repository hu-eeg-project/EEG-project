...
Nadat Data uit de Force Trainer wordt uit gelezen kan de data worden geplot op een grafiek om deze te bekijken, Hiervoor is eigen software geschreven. Als deze software gebruikt word om naar de data te kijken is te zien dat als er gefronst of met de ogen geknipperd wordt een piek te zien is in de data. Deze peik is

Als eerste is de Force Trainer opgezet en met de bijbehorende app getest. Hierbij lukte de concentratie meten bij Stefan al beter dan bij Paul. Daarna hebben we de force trainer EEG opengemaakt en draadjes op de uart aansluiting van de neurosky chip gesoldeerd. cite:force_trainer_hack Vervolgens hebben we met behulp van de documentatie van neurosky een parser geschreven die de data van de uart omzet in bruikbare data die geplot kan worden.
...
Bij het uitlezen van deze data is gezien dat er veel 50Hz int het signaal te vinden is en dat fronzen of knipperen geen pieken genereerd.
