...
* De hardware
Voor het hardware gedeelte hebben wij drie onderdelen. Hier kijken we naar twee bestaande EEGs. De Force Trainer is een goedkope speelgoed EEG die gebruikt wordt om concentratie te meten met een neurosky chip. OpenEEG is een open source project om een modulaire EEG te maken. Tenslotte ons eigen EEG is het circuit dat wij zelf proberen te maken om signalen te versterken en te meten.

** Force Trainer
Als eerste hebben we de force trainer opgezet en met de bijbehorende app getest. Hierbij lukte de concentratie meten bij Stefan al beter dan bij Paul. Daarna hebben we de force trainer EEG opengemaakt en draadjes op de uart aansluiting van de neurosky chip gesoldeerd. cite:force_trainer_hack Vervolgens hebben we met behulp van de documentatie van neurosky een parser geschreven die de data van de uart omzet in bruikbare data die geplot kan worden.
